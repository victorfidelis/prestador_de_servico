...
void main() {
  ...
  void setUpValues() {
    ...
    payment1 = Payment(id: '1', paymentType: PaymentType.money, name: 'Dinheito', isActive: true, urlIcon: '');
    payment2 = Payment(id: '2', paymentType: PaymentType.pix, name: 'Pix', isActive: true, urlIcon: '');
    payment3 = Payment(id: '3', paymentType: PaymentType.debitCard, name: 'Cartão de débito', isActive: true, urlIcon: '');
    payment4 = Payment(id: '4', paymentType: PaymentType.creditCard, name: 'Cartão de crédito', isActive: true, urlIcon: '');
    payment5 = Payment(id: '5', paymentType: PaymentType.ticket, name: 'Vale', isActive: true, urlIcon: '');
    payment6 = Payment(id: '6', paymentType: PaymentType.other, name: 'Outros', isActive: true, urlIcon: '');
    
    payment5Deleted = Payment(id: '5', paymentType: PaymentType.ticket, name: 'Vale', isActive: true, urlIcon: '', isDeleted: true);

    paymentLowestDate = Payment(id: '3', paymentType: PaymentType.debitCard, name: 'Cartão de débito', isActive: true, urlIcon: '', syncDate: DateTime(2024, 11, 5));
    paymentIntermediateDate = Payment(id: '4', paymentType: PaymentType.creditCard, name: 'Cartão de crédito', isActive: true, urlIcon: '', syncDate: DateTime(2024, 11, 10));
    paymentBiggestDate = Payment(id: '5', paymentType: PaymentType.ticket, name: 'Vale', isActive: true, urlIcon: '', syncDate: DateTime(2024, 11, 15));
    ...
    () {
      ...
      test(
        ...
        () async {
          ...
          expect(loadEither.isRight, isTrue);
          expect(loadEither.right is Unit, isTrue);
          expect(syncPaymentService.sync.dateSyncPayment,
              equals(syncPayment.dateSyncPayment));
        },
      );
    },
  );
    ...
    () {
      ...
      test(
        ...
        () async {
          ...
          when(onlineMockPaymentRepository.getUnsync(
                  dateLastSync: syncPayment.dateSyncPayment))
              .thenAnswer((_) async => Either.left(NetworkFailure(failureMessage)));
  ...
  group(
    ...
    () {
      ...
      test(
        ...
        () async {
          syncPaymentService.paymentsToSync = paymentsGetAll;
          when(offlineMockPaymentRepository.deleteById(id: anyNamed('id')))
              .thenAnswer((_) async => Either.right(unit));
          when(offlineMockPaymentRepository.existsById(id: anyNamed('id')))
              .thenAnswer((_) async => Either.right(false));
          when(offlineMockPaymentRepository.insert(payment: anyNamed('payment')))
              .thenAnswer((_) async => Either.right(payment1.id));
          when(offlineMockPaymentRepository.existsById(id: anyNamed('id')))
              .thenAnswer((_) async => Either.right(true));
          when(offlineMockPaymentRepository.update(payment: anyNamed('payment')))
              .thenAnswer((_) async => Either.right(unit));
    ...
    () {
      ...
      test(
        ...
        () async {
          ...
          when(mockSyncRepository.updatePayment(syncDate: anyNamed('syncDate')))
              .thenAnswer((_) async => Either.left(GetDatabaseFailure(failureMessage)));
    ...
    () {
      ...
      test(
        ...
        () async {
          ...
          when(mockSyncRepository.get()).thenAnswer((_) async => Either.right(Sync()));
          when(onlineMockPaymentRepository.getAll()).thenAnswer((_) async => Either.right(paymentsGetAll));
          when(offlineMockPaymentRepository.existsById(id: anyNamed('id')))
              .thenAnswer((_) async => Either.left(GetDatabaseFailure(failureMessage)));
      ...
      test(
        ...
        () async {
          ...
          when(mockSyncRepository.get()).thenAnswer((_) async => Either.right(Sync()));
          when(onlineMockPaymentRepository.getAll()).thenAnswer((_) async => Either.right(paymentsGetHasDate));
          when(offlineMockPaymentRepository.existsById(id: anyNamed('id')))
              .thenAnswer((_) async => Either.right(true));
          when(offlineMockPaymentRepository.update(payment: anyNamed('payment')))
              .thenAnswer((_) async => Either.right(unit));
